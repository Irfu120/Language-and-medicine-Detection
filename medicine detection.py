...
def resize_reference_image(img_path):
    ...
    image = cv2.imread(str(img_pathpath))
...
def resize_crop_consumer_image(img_path)
    ...
    #draw a rectangle in a large which will represent the ROI
...
def preprocess():
    ...
    for dirpath, dirnames, filnames in os.walk(root_dir):
        try:
            filnames.remove("README.md")
        except ValueError:
            pass
